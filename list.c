...
void *node_list_liberar(NodeList *nl, destroy_elementlist_function_type destroy){
    if(!nl) return NULL;
    destroy(nl->data);
    free(nl);
    return NULL;
...
void list_free(List *list){
    ...
    while (list->node != NULL) {
        ...
        node_list_liberar(pn, list->destroy_element_function);
    }
    free(list);
}
...
void * list_extractFirst(List* list){
    ...
    node_list_liberar(pn, list->destroy_element_function); /*Liberamos el nodo auxiliar*/
...
void * list_extractLast(List* list){
    ...
    if (!pe) return NULL;
    node_list_liberar(pn->next, list->destroy_element_function);
...
int list_print(FILE *fd, const List* list){
    ...
    for(i=1,c=0,c2=0,qn=(NodeList*)list->node; i<n+1 ; qn=qn->next,i++){
